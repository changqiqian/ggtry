import { Component, director, Director, geometry, Node, UITransform, _decorator } from 'cc';
...
const DFS = function (prev: Draw | null, node: Node, queue: Queue, active: boolean, level:number = 0, opacity: number = 1.0) {
    ...
    if (render) {
        ...
        let key = level+node.name; //添加层级前缀加强同名过滤
