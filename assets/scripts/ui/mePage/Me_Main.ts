...
export class Me_Main extends BaseUI {
    ...
    BindUI() {
        this.mBankBtn.SetClickCallback(() => {
            if (sys.isBrowser) {
                this.ShowLayer(
                    'common',
                    'prefab/InsertWebView',
                    true,
                    (_script) => {
                        let tempScript = _script as InsertWebView
                        let param =
                            '/?userId=' +
                            LocalPlayerData.GetInstance().Data_Uid +
                            '&token=' +
                            GameConfig.LOGIN_TOKEN +
                            '&apiUrl=http://' +
                            GameConfig.UsingIp
    ...
    EnableScroll() {
        let scrollViewContentTransform =
            this.mScrollView.content.getComponent(UITransform)
        ...
        if (requireScrollHeight > scrollViewHeight) {
            let finalSize = new Size(
                scrollViewContentTransform.width,
                requireScrollHeight
            )
