...
export class Game_Player extends BaseUI 
{
    ...
    public UpdateUIDirection()
    {
        ...
        let betActPos = this.mGame_BetAmount.node.position;
        if(posX >= getVisibleSize.width/2)
        {
            ...
            this.mGame_BetAmount.node.setPosition(-Math.abs(betActPos.x) , betActPos.y , betActPos.z);
    ...
    RestoreCards()
    {
        ...
        if(currentPlayer.uid == selfUid)
        {
            ...
            if(state <= TexasCashState.TexasCashState_PreFlopRound)
            {
                ...
                {
                    if(gameData.GetActionUid() != selfUid)
                    {
                        this.ShowCardBack();
                        return;
                    }
        ...
        if(gameData.PlayerHaveCards(currentPlayer.uid))
        {
