    ...
    execute: function () {
      ...
      _export("UIMgr", UIMgr = class UIMgr {
        ...
        Init(_loadFinish) {
          ...
          var loadingConfig = new SceneConfig(SceneType.Loading, "prefab/LoadingUI", ["loading"], resFolder);
          var loginConfig = new SceneConfig(SceneType.Login, "prefab/LoginUI", ["login"], resFolder);
          var hallConfig = new SceneConfig(SceneType.Hall, "prefab/HallUI", ["hall", "cowboy"], resFolder);
        ...
        ChangeScene(_sceneType) {
          ...
          if (this.mCurrentScene != SceneType.None) {
            var configDeleteScene = this.GetSceneConfig(this.mCurrentScene);
            this.DeleteScene(this.mCurrentScene);

            for (var i = 0; i < configDeleteScene.bundleNames.length; i++) {
              (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
                error: Error()
          ...
          var configNewScene = this.GetSceneConfig(_sceneType);
        ...
        PreloadRes(_bundleNames, _resFolders, _loadFinish) {
          var loadCount = _bundleNames.length * _resFolders.length; //console.log("total loadCount === " +loadCount);

          for (var i = 0; i < _bundleNames.length; i++) {
            (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
              error: Error()
            }), ResMgr) : ResMgr).PreloadBundle(_bundleNames[i], _bundle => {
              (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
                error: Error()
              }), ResMgr) : ResMgr).PreloadAssetsInBundle(_bundle, _resFolders, () => {
                loadCount--; //console.log("rest loadCount === " +loadCount);
        ...
        RecordLayer(_key, _node, _type) {
          var keyPair = new LayerKeyPair(_key, _node, this.mCurrentScene);
...
//# sourceMappingURL=e60ee5d388f6da562a422ec7a8e11cf8a7aa507b.js.map